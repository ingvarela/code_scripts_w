...
def make_one_image(W, H, n_rows, n_cols, margin, out_path):
    img = Image.new("RGB", (W, H), "white")
    draw = ImageDraw.Draw(img)

    # cell grid with jitter to mimic your examples
    ...
    # pick a few cells to leave empty (like the gaps in the samples)
    ...
    for r in range(n_rows):
        for c in range(n_cols):
            ...
            # mostly the same shape/color, with some variations
            ...
            rot = random.uniform(0, math.tau) if shape in ("triangle","pentagon","rect","square") else 0
...
def main():
    ...
    p.add_argument("--out_dir", type=Path, default=Path("out_shapes"), help="output folder (default: out_shapes)")
    ...
    for i in range(1, args.num_images + 1):
        ...
        make_one_image(W, H, n_rows, n_cols, args.margin, out_path)
