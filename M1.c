...
static char* http_post_form(const char *url, const char *user, const char *pass,
    ...
    curl_easy_setopt(curl, CURLOPT_USERPWD, user && pass ? (char[]){0} : NULL);
...
static void authorize_clicked(void *data, Evas_Object *obj, void *event_info) {
    ...
    ewk_view_url_set(ad->web, url);
    evas_object_show(ad->web);
}

static Eina_Bool nav_policy_cb(Evas_Object *obj, Ewk_Policy_Decision *decision,
                               Ewk_Policy_Navigation_Type type,
                               Ewk_Frame_Ref *frame, void *data) {
    appdata_s *ad = data;
    const char *url = ewk_policy_decision_url_get(decision);
    if (!url) return EINA_FALSE;
    if (strncmp(url, REDIRECT_URI, strlen(REDIRECT_URI)) == 0) {
        const char *code_pos = strstr(url, "code=");
        if (code_pos) {
            char code[512];
            sscanf(code_pos, "code=%511[^&]", code);
            dlog_print(DLOG_INFO, LOG_TAG, "Got code: %s", code);
            ...
            char *resp = http_post_form(TOKEN_URL, CLIENT_ID, CLIENT_SECRET, post);
            cJSON *j = cJSON_Parse(resp);
            const cJSON *acc = cJSON_GetObjectItem(j, "access_token");
            const cJSON *ref = cJSON_GetObjectItem(j, "refresh_token");
            if (acc && ref) {
                snprintf(ad->access_token, sizeof(ad->access_token), "%s", acc->valuestring);
                snprintf(ad->refresh_token, sizeof(ad->refresh_token), "%s", ref->valuestring);
                dlog_print(DLOG_INFO, LOG_TAG, "Access token stored");
            }
            cJSON_Delete(j);
            free(resp);
        }
        evas_object_hide(ad->web);
...
static void list_devices_clicked(void *data, Evas_Object *obj, void *event_info) {
    ...
    if (strlen(ad->access_token) == 0) {
        dlog_print(DLOG_INFO, LOG_TAG, "Not authorized yet.");
        return;
    }
    char url[512];
...
static void capture_clicked(void *data, Evas_Object *obj, void *event_info) {
    appdata_s *ad = data;
    if (strlen(ad->access_token) == 0) return;
    // Example trigger only — you would pick a real deviceId from list_devices
...
static void create_base_gui(appdata_s *ad) {
    ...
    ad->web = ewk_view_add(evas_object_evas_get(ad->win));
    evas_object_size_hint_weight_set(ad->web, EVAS_HINT_EXPAND, EVAS_HINT_EXPAND);
    ewk_view_policy_decision_navigation_policy_callback_set(ad->web, nav_policy_cb, ad);
    evas_object_hide(ad->web);
    elm_box_pack_end(ad->box, ad->web);

    evas_object_show(ad->win);
}
// --------------------------------------------

// ---------- Standard Tizen app callbacks ----------
static void win_delete_request_cb(void *data, Evas_Object *obj, void *event_info) {
    ui_app_exit();
}
static void win_back_cb(void *data, Evas_Object *obj, void *event_info) {
    appdata_s *ad = data;
    if (elm_win_lower(ad->win)) ui_app_exit();
}
static bool app_create(void *data) {
    appdata_s *ad = data;
    app_get_data_path(ad->data_path, sizeof(ad->data_path));
    snprintf(ad->capture_path, sizeof(ad->capture_path), "%s/capture.jpg", ad->data_path);
