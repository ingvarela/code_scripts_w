#!/usr/bin/env python3
"""
Pew-style infographic generator — COMPLETE (edge-to-edge, no key-points, OCR in reading order)
...
- NO "Key points" section (removed)
- Yellow–Brown–Black palette
- CHART AREA STRETCHES FULL WIDTH OF CANVAS (no left/right padding)
- OCR text saved strictly in reading order: Title -> Subtitle -> Note -> Source (wrapped lines preserved)
...
class InfographicComposer:
    ...
    def compose(self, chart_img: Image.Image, cfg: InfographicConfig, out_path: str) -> Tuple[str, List[str]]:
        ...
        # Title
        if cfg.title:
            _lines = _draw_wrapped(draw, cfg.title, self.fonts['title'], (x_left, y, x_right, y), fill=self.text_color)
            ocr_lines.extend(_lines)
            h = _measure_text_h(draw, cfg.title, self.fonts['title'], x_right - x_left)
            y += h + self.gutter

        # Subtitle (lighter grey + italic)
        if cfg.subtitle:
            _lines = _draw_wrapped(draw, cfg.subtitle, self.fonts['subtitle'], (x_left, y, x_right, y), fill=SUBTITLE_COLOR)
            ocr_lines.extend(_lines)
        ...
        if cfg.note:
            _lines = _draw_wrapped(draw, cfg.note, self.fonts['small'], (x_left, foot_y, x_right, foot_y), fill=self.text_color)
            ocr_lines.extend(_lines)
            nh = _measure_text_h(draw, cfg.note, self.fonts['small'], x_right - x_left)
            foot_y += nh + int(self.gutter * 0.5)
        if cfg.source:
            src = f"Source: {cfg.source}"
            _lines = _draw_wrapped(draw, src, self.fonts['small'], (x_left, foot_y, x_right, foot_y), fill=self.text_color)
            ocr_lines.extend(_lines)
...
def render_infographic(
    ...
    # Compose document (NO key points)
