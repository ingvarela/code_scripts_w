...
"""
simple_ruler_6in.py
Adds a 0–6 inch horizontal ruler under images.

Usage:
  python simple_ruler_6in.py /path/to/image_or_folder [--dpi 300]

Notes:
- If you pass a folder, it processes all images inside.
- Output is saved next to the original with suffix "_measured".
- The ruler is always 6 inches long (rendered at the chosen DPI).
"""
...
def process_one(path: Path, dpi: int):
    try:
        ...
        return
    # Ruler width = max(image width, 6 inches in px) so it fits neatly
    ruler_width = max(img.width, 6 * dpi)
    ruler_height = 100  # simple, fixed height
...
def main():
    ap = argparse.ArgumentParser(description="Add a fixed 0–6 inch ruler below images.")
