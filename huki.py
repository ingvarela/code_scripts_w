...
def cross_reference_files(image_folder, datasets_folder, output_csv):
    ...
    for image_filename in os.listdir(image_folder):
        ...
        name, link, retrieved_date = "", "", ""
        try:
            with open(datapackage_path, "r", encoding="utf-8") as f:
                data = json.load(f)

            # Case 1: fields directly at top level
            if "name" in data and "link" in data:
                name = data.get("name", "")
                link = data.get("link", "")
                retrieved_date = data.get("retrievedDate", "")

            # Case 2: inside "sources" or "source"
            elif "sources" in data and isinstance(data["sources"], list) and len(data["sources"]) > 0:
                src = data["sources"][0]
                name = src.get("name", "")
                link = src.get("link", "")
                retrieved_date = src.get("retrievedDate", "")
            elif "source" in data and isinstance(data["source"], list) and len(data["source"]) > 0:
                src = data["source"][0]
                name = src.get("name", "")
                link = src.get("link", "")
                retrieved_date = src.get("retrievedDate", "")
