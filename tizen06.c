...
static void
win_delete_request_cb(void *data, Evas_Object *obj, Evas_Object *event_info)
{
	ui_app_exit();
}
...
static void
_download_completed_cb(download_id id, const char *file_path, void *user_data)
{
	appdata_s *ad = user_data;

	dlog_print(DLOG_INFO, LOG_TAG, "Download complete: %s", file_path);

	// Display the downloaded image
	if (elm_image_file_set(ad->image_box, file_path, NULL) == EINA_FALSE) {
...
_load_capture_cb(void *data, Evas_Object *obj, void *event_info)
{
	...
	if (!url || strlen(url) == 0) {
		dlog_print(DLOG_WARN, LOG_TAG, "Empty URL.");
		return;
	}

	dlog_print(DLOG_INFO, LOG_TAG, "Starting download from URL: %s", url);

	int ret = download_set_url(ad->download_id, url);
	if (ret != DOWNLOAD_ERROR_NONE) {
		dlog_print(DLOG_ERROR, LOG_TAG, "Failed to set URL. Err = %d", ret);
		return;
	}

	ret = download_set_destination(ad->download_id, DOWNLOAD_IMAGE_PATH);
	if (ret != DOWNLOAD_ERROR_NONE) {
		dlog_print(DLOG_ERROR, LOG_TAG, "Failed to set destination. Err = %d", ret);
		return;
	}

	ret = download_start(ad->download_id);
	if (ret != DOWNLOAD_ERROR_NONE) {
		dlog_print(DLOG_ERROR, LOG_TAG, "Failed to start download. Err = %d", ret);
	}
}
...
create_base_gui(appdata_s *ad)
{
	...
	/* Prepare download */
	int ret = download_create(&_download_completed_cb, ad, &ad->download_id);
	if (ret != DOWNLOAD_ERROR_NONE) {
		dlog_print(DLOG_ERROR, LOG_TAG, "Failed to create download handle: %d", ret);
	}
}
