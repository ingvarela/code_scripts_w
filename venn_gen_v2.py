...
def sample_center(fit_fn, rad, placed, max_tries=4000):
    ...
    while tries < max_tries:
        ...
        if inside_canvas(cx, cy, rad) and fit_fn(cx, cy, rad) and non_overlap(cx, cy, rad, placed):
            return (cx, cy)
    raise RuntimeError("Could not place a shape without overlap")
...
def parse_args():
    p = argparse.ArgumentParser(
        description="Generate many Venn-diagram images with non-overlapping shapes strictly inside their regions. Retries on failure and keeps numbering."
    )
    ...
    p.add_argument("--seed", type=int, default=None,
                   help="Base random seed. If set, image i uses seed = base + i + retry_count.")
...
def main():
    ...
    for i in range(args.count):
        ...
        # retry loop: keep same filename/index until success
        retry = 0
        while True:
            try:
                # vary seed across retries to guarantee a different sample when base seed is provided
                seed_i = (args.seed + i + retry) if args.seed is not None else None
                generate_one(
                    out_path=out_path,
                ...
                retry += 1
                # Try again with different randomness; keep the same index/filename
                print(f"[retry {retry}] {e}. Resampling...")
