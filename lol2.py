...
rules = ["one", "majority", "two", "all", "two-consecutive", "three-consecutive"]
...
def run_evaluation(args):
    pred_rule, gt_rule, ww, step = args
    ...
    for gt_rule in rules:
        for pred_rule in rules:
            for ww in window_size:
                if ww == 1 and (gt_rule not in ["one"] or pred_rule not in ["one"]):
                    continue
                if ww == 2 and (gt_rule in ["three-consecutive"] or pred_rule in ["three-consecutive"]):
                    continue
                for step in range(1, ww + 1):
                    tasks.append((pred_rule, gt_rule, ww, step))

    # Use a pool of workers equal to the number of CPU cores
    with Pool(processes=cpu_count()) as pool:
        results = pool.map(run_evaluation, tasks)

    # Find the max F-score result
    max_fscore = 0
    max_file = ""
    for fscore, output_file in results:
