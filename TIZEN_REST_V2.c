...
// 👇 Forward declaration for log appender
...
static void show_caps_clicked(void *data, Evas_Object *obj, void *event_info) {
    appdata_s *ad = data;
    if (!ACCESS_TOKEN) {
        ui_log_append(ad, "⚠️ No valid ACCESS_TOKEN. Please refresh first.");
        return;
    }
    ...
    char *resp = http_post(url, ACCESS_TOKEN, "");
    if (resp) {
...
static Eina_Bool live_loop_cb(void *data) {
    ...
    const char *data_path = app_get_data_path();
    char save_path[512];
    snprintf(save_path, sizeof(save_path), "%scaptured_image.jpg", data_path);
...
// ====================================================
// UI CREATION — FULL ORIGINAL LAYOUT RESTORED
// ====================================================
...
static bool app_create(void *data) {
    ...
    // Keep startup logic the same; UI first so messages are visible
