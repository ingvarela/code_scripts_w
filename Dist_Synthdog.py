...
# - Exclusive per-sample effect modes:
#       canvas_grit | lowres | tone | bg_blur | clean
# - Auto-fill leftover proportion to 'clean'; effects with 0 are not generated
# - Geometric transforms (perspective/skew/rotate) always ON (unless --clean_disable_geom for 'clean')
# - NO CROPPING: skip sample if transformed canvas exceeds background
# - OCR is derived from the actually drawn text (1:1 exact visible characters)
# - Saves per-sample JSON + global SynthDog metadata + effects_summary.json (filepaths)
...
# Preserve separators while injecting intra-word spacing
...
def build_feature_schedule(n, mix_dict, rng):
    """
    Create a list of length n with entries from keys of mix_dict.
    We avoid allocating 'diff' into zero-weight non-clean categories.
    """
    ...
    # prefer to adjust into categories with positive weight; always allow 'clean'
...
while produced < additional_needed:
    ...
    mode = feature_schedule[produced]  # 'canvas_grit' | 'lowres' | 'tone' | 'bg_blur' | 'clean'
    ...
    # Build plan once on CLEAN text (draw uses spaced copy, OCR uses clean)
    ...
    # 'tone' and 'bg_blur' and 'clean' don't touch canvas here
...
# Map to the names you asked for
name_map = {
    "clean": "clean_sample",
    "canvas_grit": "grit",
    "lowres": "lowres",
