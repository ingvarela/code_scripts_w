...
// Requirements met:
//  - Uses only standard EFL widgets (no extra libs, no networking/camera)
//  - Single file sample (drop-in main.c) that includes only "project.h"
//  - Buttons, text boxes (entries), and a designated place to load/show an image
// Functionality is stubbed; wire up as you wish later.

#include "project.h"  // Your project template header (pulls in app.h, Elementary, etc.)

#define LOG_TAG "minimal_ui"
...
typedef struct {
    ...
    Evas_Object *img;        // Elm_Image placeholder (no file set yet)
...
// --- button stubs (no-op) ---
static void on_take_capture(void *data, Evas_Object *obj, void *event_info) {(void)data;(void)obj;(void)event_info;}
static void on_refresh     (void *data, Evas_Object *obj, void *event_info) {(void)data;(void)obj;(void)event_info;}
static void on_clear       (void *data, Evas_Object *obj, void *event_info) {(void)data;(void)obj;(void)event_info;}
static void on_load_token  (void *data, Evas_Object *obj, void *event_info) {(void)data;(void)obj;(void)event_info;}
...
static void ui_build(appdata_s *ad) {
    ...
    // Root vertical box
    ad->root = elm_box_add(ad->conform);
    ...
    elm_object_content_set(ad->conform, ad->root);
    evas_object_show(ad->root);

    // --- Row 1: Device entry + 3 buttons ---
    ...
    elm_object_part_text_set(ad->entry_device, "guide", "deviceId (e.g., device:demo-001)");
    ...
    // --- Row 2: Token entry + Load token button ---
    ...
    // --- Image area (designated place to show an image) ---
    ...
    elm_object_text_set(ad->status, "Ready — GUI only. Buttons & fields are stubs.");
...
static void ui_app_lang_changed(app_event_info_h event_info, void *user_data){ (void)event_info; (void)user_data; }
