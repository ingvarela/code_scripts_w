...
def compare_entries_and_frames(csv_path, json_path, frame_folder_path, output_file):
    ...
    frame_files = count_files_in_folder(frame_folder_path)
...
def check_parent_folders(csv_json_parent_folder, frame_parent_folder, output_file):
    # Get all subfolders in the frame parent folder
    frame_subfolders = [os.path.join(frame_parent_folder, d) for d in os.listdir(frame_parent_folder) if os.path.isdir(os.path.join(frame_parent_folder, d))]

    # Process each subfolder in the frame parent folder
    for frame_folder in frame_subfolders:
        # Construct paths for .csv and .json files based on the frame folder name
        folder_name = os.path.basename(frame_folder)
        csv_path = os.path.join(csv_json_parent_folder, f"{folder_name}.csv")
        json_path = os.path.join(csv_json_parent_folder, f"{folder_name}.json")

        compare_entries_and_frames(csv_path, json_path, frame_folder, output_file)

# Example usage
csv_json_parent_folder = 'C:/Users/s.varela/Desktop/Weekly Findings/W52/annotations/youtube_v3_1fps_v2/' # Replace with the path to the parent folder for .csv and .json files
frame_parent_folder = 'C:/Users/s.varela/Desktop/datasets/test_videos_YT/videos_subsampled_1fps/'  # Replace with the path to the parent folder for frame folders
