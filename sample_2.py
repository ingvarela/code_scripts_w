...
Now supports:
- --dir <folder>   : scan all .json/.jsonl files in a folder
- --recursive      : with --dir, walk subfolders
- positional files : mix with --dir if you want

Defaults:
- Inputs: ["input.json"] if nothing else provided
- target: 1366
- path-field: image
- grouping: FULL directory path (distinct by full path)
- selected copies → ./picked_images
- remainder copies → ./remainder_images
- selected ann → ./picked_annotations.json
- remainder ann → ./remainder_annotations.json
"""
...
def list_annot_files_in_dir(root: str, recursive: bool) -> List[str]:
    out: List[str] = []
    ...
    if recursive:
        ...
        for name in os.listdir(root):
            ...
            if os.path.isfile(p) and name.lower().endswith((".json", ".jsonl")):
                out.append(p)
    return sorted(out)
...
def group_key_from_path(p: str, top: Optional[int], tail: Optional[int]) -> str:
    # Default: FULL directory path (distinct even if last segment name repeats elsewhere).
...
def main():
    ...
    # NEW: directory input (with optional recursion)
    ap.add_argument("--dir", default=None, help="Directory containing .json/.jsonl annotation files (default: None).")
    ap.add_argument("--recursive", action="store_true", default=False, help="With --dir, walk subfolders (default: False).")
    # Also allow positional files (mixable with --dir). Defaults to input.json if nothing provided.
    ap.add_argument("annotations", nargs="*", help="Annotation files (.json array or .jsonl lines). Default: input.json")

    # Defaults for everything else
    ap.add_argument("--target", type=int, default=1366, help="Target total images to sample (default: 1366).")
    ...
    ap.add_argument("--out-csv", default=None, help="Optional CSV for SELECTED (default: None).")
    ...
    ap.add_argument("--out-rest-csv", default=None, help="Optional CSV for NOT-SELECTED (default: None).")
    ...
    inputs: List[str] = []
    if args.dir:
        inputs.extend(list_annot_files_in_dir(args.dir, args.recursive))
    if args.annotations:
        inputs.extend(args.annotations)
    if not inputs:
        inputs = ["input.json"]
    ...
    if not all_records:
        print("No valid records found from:", inputs, file=sys.stderr)
        sys.exit(2)
    ...
    # (No CSVs by default; set paths if you want them)
