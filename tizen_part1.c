...
static void ensure_token_dir_exists(void) {
    ...
    if (stat(TOKEN_DIR, &st) == -1) {
        ...
        dlog_print(DLOG_INFO, "ST_LOG", "📂 Created directory: %s", TOKEN_DIR);
    }
}
...
static bool initialize_token_file(appdata_s *ad) {
    ...
    token_data_t t = {0};
    struct stat st;
    bool exists = (stat(TOKEN_FILE, &st) == 0);

    if (exists) {
        ui_log_append(ad, "🔍 Found existing token.txt in permanent storage.");
        if (!read_kv_file(TOKEN_FILE, &t)) ui_log_append(ad, "⚠️ Failed to read token.txt — attempting refresh.");
        else if (validate_access_token(t.access_token)) {
            ui_log_append(ad, "✅ Access token is valid.");
            ACCESS_TOKEN = strdup(t.access_token);
            return true;
        } else if (refresh_token_c(&t)) {
    ...
    if (read_kv_file(TOKEN_FILE,&t) && refresh_token_c(&t)) {
        ...
        ui_log_append(ad,"✅ Token refreshed successfully after copy."); return true;
    } else return false;
}
...
static unsigned char* readImageToBytes(const char* filePath,size_t* size){
    ...
    *size=sz; unsigned char* buf=malloc(sz);
    fread(buf,1,sz,f); fclose(f); return buf;
}

static char* encode_base64(const unsigned char* data,size_t len,size_t* outlen){
    static const char tbl[]="ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+/";
    char* out=malloc(4*((len+2)/3)+1); size_t i,j;
    for(i=0,j=0;i<len;){
        uint32_t a=i<len?data[i++]:0,b=i<len?data[i++]:0,c=i<len?data[i++]:0;
        uint32_t t=(a<<16)|(b<<8)|c;
        out[j++]=tbl[(t>>18)&63]; out[j++]=tbl[(t>>12)&63];
        out[j++]=i>len+1?'=':tbl[(t>>6)&63]; out[j++]=i>len?'=':tbl[t&63];
...
static void take_image_capture(appdata_s *ad){
    ...
    char img_path[512]; snprintf(img_path,sizeof(img_path),"%scaptured_image.jpg",TOKEN_DIR);
    char url[512]; snprintf(url,sizeof(url),"%s/devices/%s/commands",API_BASE,DEVICE_ID);
    const char payload_refresh[]="{\"commands\":[{\"component\":\"main\",\"capability\":\"Refresh\",\"command\":\"refresh\",\"arguments