...
// ---------- CONFIG ----------
static const char* DEVICE_ID  = "286bfff3-ad00-4b6b-8c77-6f400dfa99a8";  // your device id
...
static void token_load(TokenData *t) {
    ...
    while (fscanf(fp, "%255[^=]=%4095s\n", key, val) == 2) {
        if (!strcmp(key, "client_id")) strcpy(t->client_id, val);
        else if (!strcmp(key, "client_secret")) strcpy(t->client_secret, val);
        else if (!strcmp(key, "refresh_token")) strcpy(t->refresh_token, val);
        else if (!strcmp(key, "access_token")) strcpy(t->access_token, val);
        else if (!strcmp(key, "auth_code")) strcpy(t->auth_code, val);
    }
    fclose(fp);
    t->loaded = true;
}
...
static const char* get_access_token(appdata_s *ad) {
    ...
    if (strlen(TOKENS.access_token) == 0) {
        ui_log_append(ad, "No token found, using authorization code...");
...
static bool perform_api_call(appdata_s *ad, const char *url,
    ...
    if (code == 401) {
        ui_log_append(ad, "Access expired, refreshing...");
...
static void show_caps_clicked(void *data, Evas_Object *obj, void *event_info) {
    ...
    char *resp = NULL;
    if (perform_api_call(ad, url, NULL, &resp)) ui_log_append(ad, resp);
    else ui_log_append(ad, "Failed to get capabilities.");
    free(resp);
}
