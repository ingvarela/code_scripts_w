...
def layout_grid(canvas: Image.Image, icons: List[Image.Image], src_paths: List[Path],
    ...
    """
    Tiled grid; each cell centers one icon.
    PER-ICON UNIFORMITY: For each distinct icon used in THIS grid image,
    we pick one rotation and one target size and reuse the rendered bitmap.
    If uniform_size=True, all cells share the same target interior box size.
    """
    W, H = canvas.size
    cell_w = W // C
    cell_h = H // R
    ...
    # Decide a single target box if uniform_size; else we will choose a per-icon target.
    if uniform_size:
        target_w_global = int(max_cell_w * max(0.0, min(1.0, uniform_fill)))
        target_h_global = int(max_cell_h * max(0.0, min(1.0, uniform_fill)))

    # Pre-render cache: icon_idx -> rendered Image (same rotation & size reused)
    icon_cache: Dict[int, Image.Image] = {}

    # Decide per-icon transforms and render once
    distinct_icon_indices = sorted(set(picks))
    for idx_icon in distinct_icon_indices:
        base = icons[idx_icon]
        # one rotation per icon for this image
        rot_deg = random.uniform(*rot_range)

        if uniform_size:
            target_w, target_h = target_w_global, target_h_global
        else:
            # choose ONE fill factor per icon, then fit to that target for consistency
            fill = random.uniform(*scale_fill_range)
            target_w = int(max_cell_w * fill)
            target_h = int(max_cell_h * fill)

        im = rotate_and_scale_fit_exact(base, rot_deg, target_w, target_h)
        if im is None:
            # as a fallback, try without rotation and smaller target
            im = rotate_and_scale_fit_exact(base, 0.0, max(1, target_w // 2), max(1, target_h // 2))
            if im is None:
                continue
        icon_cache[idx_icon] = im

    # Place cells using cached bitmaps (guarantees per-icon uniformity)
    for idx_cell in range(total):
        r = idx_cell // C
        c = idx_cell % C
        idx_icon = picks[idx_cell]
        if idx_icon not in icon_cache:
            continue  # skip if something failed to render
        im = icon_cache[idx_icon]
...
def main():
    ...
    # scaling / rotation (user prefs; grids enforce per-icon consistency)
    ...
    # optional: same target for ALL cells in a grid
    ap.add_argument("--grid-uniform-size", action="store_true",
                    help="If set, all cells in a grid use the same target size (in addition to per-icon uniformity).")
    ...
    print("Tips: --bg random, --rot-max 25 for variety; "
          ...
          "--grid-uniform-size for same-sized icons across ALL cells.")
